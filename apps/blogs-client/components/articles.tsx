...
const Articles = () =>{
    ...
    const articles = trpc.blogs.getBlogs.useQuery().data;
    ...
    return <>
        {
            <div className="w-full h-full mt-10">
                {articles && articles.map(item =>                   
                        <div key={item.title} className="block w-full h-full p-10 m-2 text-center w-64 bg-[#F6F5F5]">
                            <Link href={{pathname: '/article', query: {name: item.title, imgSrc: item.imageSrc}}} >
                                {item.title}
                            </Link>
                        </div>
